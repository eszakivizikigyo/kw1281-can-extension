...
public class CanAutoScanTests
{
    ...
    {
        Assert.AreEqual("Ecu", Program.GetControllerName(0x01));
    }

    [TestMethod]
    public void GetControllerName_Cluster_ReturnsCluster()
    {
        Assert.AreEqual("Cluster", Program.GetControllerName(0x17));
    }

    [TestMethod]
    public void GetControllerName_CanGateway_ReturnsCanGateway()
    {
        Assert.AreEqual("CanGateway", Program.GetControllerName(0x19));
    }

    [TestMethod]
    public void GetControllerName_Radio_ReturnsRadio()
    {
        Assert.AreEqual("Radio", Program.GetControllerName(0x56));
    }

    [TestMethod]
    public void GetControllerName_UnknownAddress_ReturnsModuleHex()
    {
        Assert.AreEqual("Module 0x42", Program.GetControllerName(0x42));
    }

    [TestMethod]
    public void GetControllerName_AllKnownAddresses_ReturnNonEmptyNames()
    {
        byte[] knownAddresses = { 0x01, 0x09, 0x17, 0x19, 0x25, 0x35, 0x37, 0x46, 0x56, 0x7C };
        foreach (var address in knownAddresses)
        {
            var name = Program.GetControllerName(address);
    ...
    public void GetControllerName_AddressZero_ReturnsModuleHex()
    {
        ...
        Assert.AreEqual("Module 0x00", Program.GetControllerName(0x00));
    }

    [TestMethod]
    public void GetControllerName_MaxAddress_ReturnsModuleHex()
    {
        Assert.AreEqual("Module 0x7F", Program.GetControllerName(0x7F));
    }
}
